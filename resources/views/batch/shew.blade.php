...
<body>
            ...
            <div class="col-md-3 mb-4">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-text display-4">{{ count($sensorIds ?? []) }}</p>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-4">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-text display-4">{{ $excursionCount ?? 0 }}</p>
                    </div>
                </div>
            </div>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach ($latestReadings as $reading)
                                <tr class="{{ $reading['hasExcursion'] ? 'table-danger' : '' }}">
                                    <td>{{ $reading['sensorId'] }}</td>
                                    <td>{{ \Carbon\Carbon::parse($reading['timestamp'])->format('Y-m-d H:i:s') }}</td>
                                    <td>{{ $reading['decryptedValue'] }}</td>
                                    <td>{{ $reading['hasExcursion'] ? 'Yes' : 'No' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    <script>
        ...
        new Chart(document.getElementById('lineChart').getContext('2d'), {
            type: 'line',
            data: {
                labels: @json($lineChartData['timestamps']),
        ...
        new Chart(document.getElementById('gaugeChart').getContext('2d'), {
            ...
            data: {
                datasets: [{
                    data: [@json($gaugeData['value']), 100 - @json($gaugeData['value'])],
