{{-- resources/views/sensors/monitor.blade.php --}}
@extends('layouts.app')

@section('title', 'Sensor Dashboard - ' . $batchId)
...
@section('content')
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-1">
                            <p class="text-sm text-red-700">
                                {{ $excursions }} sensor{{ $excursions > 1 ? 's' : '' }} showing excursion values. 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div class="metric-card">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800 mt-2">{{ $uniqueSensors }}</p>
                </div>
        ...
        {{-- Excursions with Badge --}}
        <div class="metric-card {{ $excursions > 0 ? 'ring-2 ring-red-500 ring-opacity-50' : '' }}">
            <div class="flex justify-between items-start">
                <div>
                    <div class="flex items-center">
                    ...
                    <p class="text-2xl font-bold {{ $excursions > 0 ? 'text-red-600' : 'text-gray-800' }} mt-2">
                        {{ $excursions }}
                    </p>
                </div>
                <div class="{{ $excursions > 0 ? 'bg-red-50' : 'bg-gray-50' }} rounded-full p-2">
                    <svg class="w-6 h-6 {{ $excursions > 0 ? 'text-red-500' : 'text-gray-400' }}" 
            ...
            @if($excursions > 0)
                <div class="mt-4 text-sm text-red-600">
                    Last excursion: {{ \Carbon\Carbon::parse($latestExcursion['timestamp'])->diffForHumans() }}
                </div>
            @endif
        </div>
            ...
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800 mt-2">
                        {{ number_format(collect($temperatureReadings)->last()['value'], 1) }}°C
                    </p>
                </div>
    ...
    {{-- Readings Table with Enhanced Excursion Indicators --}}
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($latestReadings as $reading)
                        <tr class="{{ $reading['hasExcursion'] ? 'table-row-excursion' : '' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ ucfirst($reading['readingType']) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ number_format($reading['value'], 2) }}
                                {{ $reading['readingType'] === 'temperature' ? '°C' : 
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                @if($reading['hasExcursion'])
                                    <span class="inline-flex items-center">
                                        ...
                                        <button onclick="showExcursionDetails('{{ $reading['id'] }}')" 
            ...
            <div class="mt-3 text-center">
                ...
                <div class="mt-4 px-7 py-3">
                    <div class="text-sm text-gray-500 text-left" id="excursionDetails">
                        {{-- Excursion details will be populated via JavaScript --}}
                    </div>
                </div>
...
<script>
    ...
    new Chart(ctx, {
        type: 'line',
        data: {
            labels: {!! json_encode(collect($temperatureReadings)->pluck('timestamp')) !!},
            datasets: [{
                label: 'Temperature (°C)',
                data: {!! json_encode(collect($temperatureReadings)->pluck('value')) !!},
                borderColor: '#3b82f6',
    ...
    function showExcursionDetails(readingId) {
        const reading = {!! json_encode($latestReadings) !!}.find(r => r.id === readingId);
        if (reading) {
